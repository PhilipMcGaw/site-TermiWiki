---
type: research-record
status: lead
subject: "Ross Childs / Rictus — LinkedIn search"
date: 2026-09-19
source_type: public-linkedin-search
confidence: high for candidate exclusion; unverified for wider identity search
---

# Ross Childs / Rictus — LinkedIn search

## Search and retrieval

- **Searches:** `Ross Childs`, `Rictus Ross`, and combinations with `Plymouth`, `Plymouth University`, `TermiSoc`, and `Flux`.
- **Retrieved:** 2026-09-19.
- **Access:** Public search-result snippets; no private LinkedIn content was accessed.

## Results

LinkedIn returned multiple people named Ross Childs, including:

- [Ross Childs — Three UK](https://uk.linkedin.com/in/rossdchilds), London/Reading area, described in the indexed snippet as a technology leader;
- [Ross Childs — St. Columba's College](https://uk.linkedin.com/in/ross-childs-a465908), Buckinghamshire, with education at the University of Buckingham;
- other same-name profiles in the United Kingdom, the United States, Australia, and elsewhere.

The indexed results did not expose a reference to TermiSoc, the University of Plymouth, Flux BBS, the 1997–98 Projects Officer role, or the alias “Rictus”. No identity match is therefore established.

The candidate profile was then inspected by the project maintainer. It lists [West Hatch High School](https://www.westhatch.net/) from 1993 to 2000 and an initial job in the London area from 2000. This education and early-career geography conflicts with the historical Ross Childs, who is documented in Plymouth TermiSoc material as a 1996 member and 1997–98 Projects Officer. The profile should therefore be treated as a likely same-name false match.

## Interpretation

The Three UK profile was initially a plausible lead only because it was a UK technology profile under the exact name Ross Childs. The school and early-career details now make it a likely false match. The other visible profiles have no stronger connection in the indexed material.

This search adds a set of candidate URLs for future comparison, but it does not add publishable biographical evidence about Ross Childs.

## Unresolved questions

- [x] TODO: Compare the leading candidate against the available historical chronology; the West Hatch/London timeline is inconsistent with the Plymouth TermiSoc chronology.
- [ ] TODO: Compare any remaining candidate profile against independently supplied identity details or a direct response from Ross; do not infer identity from name, location, or profession alone.
- [ ] TODO: Re-run a public search for `Rictus` and Ross Childs if a distinctive modern username, employer, or location becomes available.

## Confidence

- **High:** LinkedIn exposes multiple same-name candidates and no indexed result directly connects one to TermiSoc.
- **High:** The Three UK profile is a likely same-name false match based on the school and early-career timeline supplied by the project maintainer, compared with the documented Plymouth chronology.
- **Unverified:** The identity of the historical Ross “Rictus” Childs on LinkedIn.
