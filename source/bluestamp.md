---
title: "BlueStamp"
source: dokuwiki
source_path: "sites:termiwiki:bluestamp"
created: 2023-01-01
tags:
  - "termisoc"
  - "projects"
  - "bluestamp"
  - "relation"
  - "references"
  - "rich_jeffery"
  - "ben_a_lee"
  - "gemma_peter"
  - "edward_mccaughan"
  - "damien_hopton"
  - "philip_mcgaw"
  - "carl_notman"
  - "seth_kneller"
  - "things_to_consider"
  - "2006-06-01_-_meeting_notes"
---
# BlueStamp

BlueStamp Technologies was a plan to run a business offshoot of [TermiSoc](termisoc.md) (whilst, in itself not being an offshoot in the union's eyes). It's codename: TermiSoc Inc.

Yep, several members went off and tried starting up a company in which to do all the things TermiSoc could do if they weren't part of the student onion (like for example the Union website, hosting for external organisations, alumni, etc.)

After several abortive meetings to decide what BlueStamp would do (with unnamed people trying to add ridiculous ideas, people trying to take the discussions way, way ahead of where it should have been and more than one person lumping their company's assets in with ours), 2 years where nothing happened, and over £500 accrued in debts due to an issue involving who was filling in the tax return that year, the company was closed down in late 2008.

## Who was involved?
The company was the brainchild of [Rich](TermiPeople/rich_jeffery.md) and [Ben](TermiPeople/ben_a_lee.md), with [Gemma](TermiPeople/gemma_peter.md) chipping in from the start.

The company also had input from:
- [McCaughan](TermiPeople/edward_mccaughan.md)
- [Hopton](TermiPeople/damien_hopton.md)
- [McGaw](TermiPeople/philip_mcgaw.md)
- [Notman](TermiPeople/carl_notman.md)
- [Kneller](TermiPeople/seth_kneller.md)

# And who was actually in charge?
According to the official paperwork, the partners in BlueStamp Technologies, LLP. were:
- [Jeffery](TermiPeople/rich_jeffery.md)
- [A'Lee](TermiPeople/ben_a_lee.md)
- [Peter](TermiPeople/gemma_peter.md)
- [McCaughan](TermiPeople/edward_mccaughan.md)
- [Hopton](TermiPeople/damien_hopton.md)

# Why BlueStamp?
In a word, why not?

Seriously, the members voted for the best name out of the choices and it came out first (well, actually it came second. Sinister Technologies as a name will be used for an offshoot of BlueStamp).

# Is there any salvage from the wreckage?
Through the magic of the internet, we can present two previously lost documents for your enjoyment. Maybe someone at some point can explain what went wrong…
- "[To Consider](things_to_consider.md)", dated 1st June 2006
- [- Meeting Notes](2006-06-01_-_meeting_notes.md) 
